...
@Autonomous(name="AutonomusBlueFar", group="Far")
public class AutonomusBlueFar extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            FtcDashboard.getInstance().getTelemetry().addData("YawErr", Vehicles.getInstance().yawErr);  TransferBall.getInstance().startFlow();
            TransferBall.getInstance().stopBrush();
            TransferBall.getInstance().stopFlow();
            GunControl.getInstance().startShot();
            TransferBall.getInstance().setVelFlow(0);
...
//        Vehicles.getInstance().goTo(vehicles.targetX, vehicles.targetY, vehicles.targetYaw);
            ...
            FtcDashboard.getInstance().getTelemetry().update();
            while (Vehicles.getInstance().goTo(-20, 0, 0) && opModeIsActive()) {
                Vehicles.getInstance().goTo(-20, 0, 0);
                GunControl.getInstance().startShot();
                TransferBall.getInstance().setVelFlow(0);
                TransferBall.getInstance().stopFlow();
                gunConfig.bananPosition = 0.8;
                GunControl.getInstance().setBananServo();
                GunControl.getInstance().setTowerDegree(OdometerPinpoint.getInstance().getYaw()  - 30, gunConfig.offsetGun, gunConfig.distanceToTarget);
                FieldView.renderRobot(Vehicles.getInstance().getPositionOdometerX() - positionAutoRed.xSwap, Vehicles.getInstance().getPositionOdometerY() + positionAutoRed.ySwap, OdometerPinpoint.getInstance().getYaw() + positionAutoRed.yawSwap);
            }
            ...
            while (Vehicles.getInstance().goTo(-30, 0,  -90) && opModeIsActive()) {
                Vehicles.getInstance().goTo(-30, 0, -90);
            }
            runtime.reset();
            TransferBall.getInstance().setVelFlow(1);
